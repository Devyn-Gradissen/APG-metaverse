...
namespace FiveMinuteChat
{
    public static class GUILayoutUtils
    {
        ...
        public static bool LinkLabel( string labelText, Color labelColor, Vector2 contentOffset, int fontSize, string webAddress )
        {
            if( LinkLabel( labelText, labelColor, contentOffset, fontSize ) )
            {
                ...
                {
                    Logger.LogError( "Could not open URL. Please check your network connection and ensure the web address is correct." );
    ...
    [CustomEditor( typeof(ChatConnectionBehavior) )]
    public class ChatConnectionBehaviorEditor : Editor
    {
        ...
        private bool _isRunningRequests = false;

        public override void OnInspectorGUI()
        {
            serializedObject.Update();

            var targetBehavior = (ChatConnectionBehavior)target;
            ...
            var dontDestroyOnLoad = targetBehavior.DontDestroyOnLoad;
            targetBehavior.DontDestroyOnLoad = EditorGUILayout.Toggle( "Don't destroy on load", dontDestroyOnLoad );
            ...
            if( targetBehavior.ApplicationName != previousApplicationName ||
                ...
                targetBehavior.DontDestroyOnLoad != dontDestroyOnLoad ||
        ...
        public async Task RequestTrialApplication( ChatConnectionBehavior targetBehavior )
        {
            if( _isRunningRequests )
            {
                Logger.LogWarning( "Already waiting for requests to finish." );
                return;
            }
            ...
            try
            {
                ...
                if( await targetBehavior.DiscoverServers() )
                {
                    ...
                    {
                        Logger.Log( "Application credentials were successfully generated." );
        ...
        private async Task<bool> DoRequestTrialApplicationRequest( ChatConnectionBehavior targetBehavior )
        {
            using( var req =
        ...
        private bool RequestTrialApplicationResult( UnityWebRequest req, ChatConnectionBehavior targetBehavior )
        {
            ...
            Logger.LogError( $"Trial application credentials could not be acquired: {req.error}" );
            return false;
        }
    }
}
